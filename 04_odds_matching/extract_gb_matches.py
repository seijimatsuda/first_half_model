#!/usr/bin/env python3
"""
Extract all matches from GB_24_25.tar dataset
"""
                    ...
                    if lines:
                        ...
                        if 'mc' in first_line_data and first_line_data['mc']:
                            ...
                            if market_definition:
                                ...
                                if event_name and " v " in event_name and market_type == "OVER_UNDER_05":
                                    # This is a first half over/under 0.5 goals market
                                    match_count += 1
                                            ...
                                            for market_change in data['mc']:
                                                if 'rc' in market_change:
                                                    for runner_change in market_change['rc']:
                                                        if runner_change.get('id') == 5851482 and 'ltp' in runner_change:  # Under 0.5 Goals
                                                            under_05_odds = runner_change['ltp']
                                                        if runner_change.get('id') == 5851483 and 'ltp' in runner_change:  # Over 0.5 Goals
...
def analyze_matches(matches):
    ...
    # Show unique event names (to see team combinations)
    print(f"\n=== UNIQUE EVENT NAMES (first 50) ===")
    unique_events = df['event_name'].unique()
    for i, event in enumerate(unique_events[:50]):
        print(f"{i+1:2d}. {event}")
    
    if len(unique_events) > 50:
        print(f"... and {len(unique_events) - 50} more events")
