"""Realistic backtesting that processes matches chronologically."""
...
class RealisticBacktestResult:
    """Result of a realistic backtest bet."""
    fixture_id: str
    match_date: datetime
    home_team: str
    away_team: str
...
class RealisticBacktester:
    ...
    def run_backtest(
        ...
        # Get fixtures to backtest in chronological order
        fixtures = self._get_fixtures_for_backtest(start_date, end_date, league_filter)
        self.total_fixtures = len(fixtures)
        print(f"📊 Found {self.total_fixtures} fixtures to backtest")
        
        if not fixtures:
            print("❌ No fixtures found for backtest")
            return
        
        # Process each fixture chronologically
        for i, fixture in enumerate(fixtures):
            if i % 20 == 0:
                print(f"Processing fixture {i+1}/{self.total_fixtures}...")
            
            result = self._backtest_fixture_realistic(fixture, min_samples_home, min_samples_away)
            if result:
                self.results.append(result)
                self.processed_fixtures += 1
    ...
    def _get_fixtures_for_backtest(
        ...
        """Get fixtures for backtesting in chronological order."""
    ...
    def _backtest_fixture_realistic(
        self,
        fixture: Fixture,
        min_samples_home: int,
        min_samples_away: int
    ) -> Optional[RealisticBacktestResult]:
        """Backtest a single fixture using only data available before the match."""
        
        try:
            # Get historical samples for both teams (only before this match)
            home_samples, away_samples = self._get_historical_samples_realistic(
    ...
    def _get_historical_samples_realistic(
        self,
        fixture: Fixture,
        min_samples_home: int,
        min_samples_away: int
...
def run_realistic_backtest(
    ...
    """Run realistic backtest with command line interface."""
...
if __name__ == "__main__":
    ...
    run_realistic_backtest(
        ...
        export_file="realistic_backtest_results.csv",
        min_samples=5
    )
