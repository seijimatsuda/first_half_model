#!/usr/bin/env python3
"""
Betfair Historical Data Parser and PnL Calculator

This module parses Betfair historical odds data from tar archives and matches
it with model predictions to calculate accurate PnL using real historical odds.
"""
...
class BetfairHistoricalParser:
    """Parser for Betfair historical odds data."""
    
    def __init__(self, tar_path: str, temp_dir: str = "temp_odds"):
    ...
    def extract_market_data(self, records: List[Dict]) -> Dict:
        """Extract relevant market data from Betfair records."""
        market_data = {}
        ...
        for record in records:
            ...
            for market in mc:
                ...
                if 'marketDefinition' in market:
                    ...
                    # Only process First Half Goals 0.5 markets
                    if 'FH_GOAL_05' in market_type:
    ...
    def parse_all_files(self) -> None:
        ...
        print("Parsing all Betfair data files...")
        ...
        for i, file_path in enumerate(bz2_files):
            ...
            market_data = self.extract_market_data(records)
            all_market_data.update(market_data)
        
        self.odds_data = all_market_data
        print(f"Parsed {len(self.odds_data)} unique markets")
    
    def get_final_odds(self, market_id: str) -> Optional[Dict[str, float]]:
        """Get the final odds for a market before kickoff."""
        ...
        # Convert to final odds format
        for runner_id, price_data in pre_kickoff_prices.items():
            runner_name = market['runners'].get(runner_id, f'Runner_{runner_id}')
            final_odds[runner_name] = price_data['price']
        
        return final_odds if final_odds else None
    
    def parse_predictions_text(self, filepath: str) -> pd.DataFrame:
        """Parse the text format predictions file."""
        data = []
        with open(filepath, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Fixed pattern: matches both ✅ WIN and ❌ LOSS
    ...
    def normalize_team_name(self, name: str) -> str:
        ...
        # Direct mapping
        for normalized, variants in team_mappings.items():
            if name in variants:
                return normalized
    ...
    def match_events_to_predictions(self, predictions_df: pd.DataFrame) -> pd.DataFrame:
        """Match Betfair events to model predictions."""
        print("Matching Betfair events to model predictions...")
        ...
        for _, pred in predictions_df.iterrows():
            ...
            for market_id, market in self.odds_data.items():
                ...
                if abs((market_time - pred_date).days) <= 1:
                    # Try to match team names in event name
                    event_lower = event_name.lower()
                    home_lower = home_team.lower()
                    away_lower = away_team.lower()
                    
                    score = 0
                    if home_lower in event_lower:
                        score += 1
                    if away_lower in event_lower:
                        score += 1
                    
                    # Check for common team name variations
                    if 'arsenal' in event_lower and 'arsenal' in home_lower:
                        score += 0.5
                    if 'man city' in event_lower and 'manchester city' in home_lower:
                        score += 0.5
                    if 'man utd' in event_lower and 'manchester united' in home_lower:
                        score += 0.5
            ...
            if best_match and best_score >= 1.5:  # Require at least partial match
                # Get final odds for this market
                final_odds = self.get_final_odds(best_match['market_id'])
                
                if final_odds and 'Over 0.5 Goals' in final_odds:
                    ...
                    matched_pred['betfair_odds'] = final_odds['Over 0.5 Goals']
                    matched_pred['match_score'] = best_score
                    matched_predictions.append(matched_pred)
                else:
                    print(f"No odds found for {home_team} vs {away_team} on {pred_date}")
            else:
                print(f"No Betfair match found for {home_team} vs {away_team} on {pred_date}")
        
        return pd.DataFrame(matched_predictions)
    
    def calculate_pnl(self, matched_predictions: pd.DataFrame, stake: float = 100.0) -> Dict:
        """Calculate PnL using real Betfair odds."""
        print(f"Calculating PnL with ${stake} flat stakes...")
    ...
    def run_full_analysis(self, predictions_file: str) -> Dict:
        ...
        print("Starting Betfair historical odds analysis...")
        ...
        if predictions_file.endswith('.txt'):
            # Parse the text format
            predictions_df = self.parse_predictions_text(predictions_file)
        else:
            predictions_df = pd.read_csv(predictions_file)
        
        # Match events and calculate PnL
        matched_predictions = self.match_events_to_predictions(predictions_df)
        pnl_results = self.calculate_pnl(matched_predictions)
        
        print(f"\n📊 BETFAIR PnL ANALYSIS RESULTS")
        print("=" * 50)
...
def main():
    ...
    parser = BetfairHistoricalParser("data/data (1).tar")
    
    # Run analysis on the matchweek 5+ predictions
    results = parser.run_full_analysis("matchweek5_plus_predictions.txt")
    
    # Save detailed results
    bet_details_df = pd.DataFrame(results['bet_details'])
    bet_details_df.to_csv("betfair_pnl_analysis.csv", index=False)
    print(f"\nDetailed results saved to: betfair_pnl_analysis.csv")
