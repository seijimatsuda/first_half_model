#!/usr/bin/env python3
"""
Bet Reconciliation System

Reconciles model prediction files with actual bet spreadsheet to identify
...
class BetReconciler:
    ...
    def _build_alias_map(self) -> Dict[str, str]:
        ...
        return {
            ...
            "manchester city": "manchester city",  # Duplicate for consistency
    ...
    def parse_predictions_simple_format(self, filepath: str) -> pd.DataFrame:
        """Parse simple format like: WIN TeamA vs TeamB (YYYY-MM-DD)"""
        data = []
        
        with open(filepath, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Pattern for: WIN TeamA vs TeamB (YYYY-MM-DD)
        pattern = r'(WIN|LOSS)\s+([A-Za-z\' .]+)\s+vs\s+([A-Za-z\' .]+)\s+\((\d{4}-\d{2}-\d{2})\)'
    ...
    def parse_predictions_week_format(self, filepath: str) -> pd.DataFrame:
        """Parse format with week headers and bet lines."""
        data = []
        ...
        # Pattern for: 2024-08-17 - West Ham vs Aston Villa
        pattern = r'(\d{4}-\d{2}-\d{2})\s*-\s*([A-Za-z\' .]+)\s+vs\s+([A-Za-z\' .]+)'
        ...
        for match in re.finditer(pattern, content):
            ...
            try:
                ...
                data.append({
                    ...
                    'Home Team': home_team,
                    'Away Team': away_team,
                    'ModelResult': 'WIN'  # Assume WIN for this format
                })
            except ValueError:
                continue
    ...
    def reconcile(self, pred_df: pd.DataFrame, actual_df: pd.DataFrame) -> Dict:
        """Reconcile predictions with actual bets."""
        # Add normalized team names to predictions
        pred_df = pred_df.copy()
        pred_df['Home_clean'] = pred_df['Home Team'].apply(self.normalize_team)
        pred_df['Away_team'] = pred_df['Away Team'].apply(self.normalize_team)
        
        # Create join keys
        pred_df['join_key'] = pred_df['Date'].astype(str) + '|' + pred_df['Home_clean'] + '|' + pred_df['Away_team']
    ...
    def _classify_discrepancies(self, df: pd.DataFrame, other_df: pd.DataFrame, source: str) -> pd.DataFrame:
        ...
        for idx, row in df.iterrows():
            ...
            away_clean = row['Away_team'] if source == 'model' else row['Away_clean']
                ...
                if len(same_date) > 0:
                    ...
                    for _, other_row in same_date.iterrows():
                        if source == 'model':
                            ...
                            other_home = other_row['Home_clean']
                            other_away = other_row['Away_team']
    ...
    def detect_selection_rule(self, df: pd.DataFrame, filepath: str) -> Dict:
        """Detect selection rule used in the file."""
        rule_info = {
            'threshold': None,
            'rule_type': 'unknown',
            'confidence': 'low'
        ...
        # Read raw file to look for threshold indicators
        with open(filepath, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Look for combined average thresholds
        combined_avg_pattern = r'combined.*?(\d+\.?\d*)'
        matches = re.findall(combined_avg_pattern, content.lower())
        if matches:
            # Find the most common threshold
            thresholds = [float(m) for m in matches if '.' in m]
            if thresholds:
                rule_info['threshold'] = max(set(thresholds), key=thresholds.count)
                rule_info['rule_type'] = 'combined_average'
                rule_info['confidence'] = 'medium'
        
        # Look for probability thresholds
        prob_pattern = r'p\(.*?(\d+\.?\d*)'
        prob_matches = re.findall(prob_pattern, content.lower())
        if prob_matches:
            rule_info['rule_type'] = 'probability'
            rule_info['confidence'] = 'medium'
        
        return rule_info
    
    def run_reconciliation(self, model_files: List[str], actual_file: str) -> Dict:
        ...
        for model_file in model_files:
            ...
            elif 'simple_algorithm' in model_file or 'all_predictions' in model_file:
                pred_df = self.parse_predictions_simple_format(model_file)
            else:
                pred_df = self.parse_predictions_week_format(model_file)
            
            print(f"Parsed {len(pred_df)} predictions")
            
            # Reconcile
            reconciliation = self.reconcile(pred_df, actual_bets)
            
            # Detect selection rule
            rule_info = self.detect_selection_rule(pred_df, model_file)
            
            # Save results
            base_name = Path(model_file).stem
            
            # Save overlap
            overlap_file = f"/tmp/{base_name}_overlap.csv"
            reconciliation['perfect_matches'].to_csv(overlap_file, index=False)
            
            # Save model-only
            model_only_file = f"/tmp/{base_name}_model_only.csv"
            reconciliation['model_only'].to_csv(model_only_file, index=False)
            
            # Save actual-only
            actual_only_file = f"/tmp/{base_name}_actual_only.csv"
            reconciliation['actual_only'].to_csv(actual_only_file, index=False)
            ...
            results[model_file] = {
                ...
                'files': {
                    ...
                    'model_only': model_only_file,
                    'actual_only': actual_only_file
                }
            }
    ...
    def generate_summary(self, results: Dict) -> str:
        """Generate markdown summary of reconciliation results."""
        summary = "# Bet Reconciliation Summary\n\n"
        ...
        for model_file, result in results.items():
            ...
            if rule['threshold']:
                summary += f"- **Detected threshold**: {rule['threshold']} ({rule['rule_type']})\n"
...
def main():
    ...
    # Run reconciliation
    reconciler = BetReconciler()
    results = reconciler.run_reconciliation(model_files, actual_file)
    
    # Generate summary
    summary = reconciler.generate_summary(results)
    print(summary)
    
    # Save summary
    with open('/tmp/reconciliation_summary.md', 'w') as f:
        f.write(summary)
    
    print("Reconciliation complete! Check /tmp/ for detailed CSV files.")
